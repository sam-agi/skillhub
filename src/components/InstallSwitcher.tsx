...
export function InstallSwitcher({ exampleSlug = "sonoscli" }: InstallSwitcherProps) {
  ...
  const command = useMemo(() => {
    switch (pm) {
      case "npm":
        return `npx @bgicli/skillhub@latest install ${exampleSlug}`;
      case "pnpm":
        return `pnpm dlx @bgicli/skillhub@latest install ${exampleSlug}`;
      case "bun":
        return `bunx @bgicli/skillhub@latest install ${exampleSlug}`;
    }
  }, [exampleSlug, pm]);
