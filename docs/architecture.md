...
- CLI: `packages/skillhub/` (published as `skillhub`, legacy `clawdhub`).
...
- Persist install state:
  - `./.skillhub/lock.json` (per workdir, legacy `.clawdhub`)
  - `./skills/<slug>/.skillhub/origin.json` (per skill folder, legacy `.clawdhub`)
